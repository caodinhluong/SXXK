...
import {
    ...
    InputNumber, Card, Typography, Row, Col, Space, message, Modal
} from "antd";
import {
...
} from "../../services/tokhaixuat.service";
import { getAllLoHang, createLoHang } from "../../services/lohang.service";
...
const NhapToKhaiXuat = () => {
    ...
    const handleImportExcel = async (file) => {
        try {
            ...
            if (importFormat === "hai_quan") {
                ...
                res = await importToKhaiFromHaiQuanExcel(parsedRow, selectedLoHang, true);
            } else {
                res = await importToKhaiXuatFromExcel(jsonData, selectedLoHang);
            }
            
            if (res.success) {
                message.success(`Import thành công: ${res.data?.thanh_cong || 0} tờ khai`);
                setImportModalVisible(false);
                const resTKX = await getAllToKhaiXuat();
                setToKhaiList(Array.isArray(resTKX) ? resTKX : (resTKX.data || []));
            } else {
    ...
    const handleDownloadTemplate = async () => {
        try {
            const res = await getTemplateToKhaiXuat();
            if (res.success && res.data) {
                const templateData = res.data.vi_du ? [res.data.vi_du] : [];
                const ws = XLSX.utils.json_to_sheet(templateData);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "ToKhaiXuat");
                XLSX.writeFile(wb, "_template_tokhai_xuat.xlsx");
    ...
    const onFinish = async () => {
        try {
            ...
            console.log(">>> Payload Lô hàng:", payloadLoHang);
            const resLoHang = await createLoHang(payloadLoHang);
            console.log(">>> Response Lô hàng:", resLoHang);
            ...
            console.log(">>> Payload Hóa đơn:", payloadHoaDon);
            ...
            showSaveError('tờ khai xuất', errorMessage);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <>
            ...
            {/* Import Excel Modal */}
            <Modal
                title="Import Tờ khai Xuất từ Excel"
                    ...
                    <Form layout="vertical">
                        ...
                        {importFormat === "he_thong" && (
                            <p style={{ marginBottom: 16, color: "#666" }}>
                                File Excel cần có các cột: <b>so_tk, ngay_tk, ma_to_khai, loai_hang, cang_xuat, tong_tri_gia</b>
                            </p>
                        )}
                        ...
                        {importFormat === "hai_quan" && (
                            <div style={{ marginBottom: 16, padding: 12, background: "#f5f5f5", borderRadius: 6 }}>
                                ...
                                <ul style={{ marginBottom: 0, paddingLeft: 16, color: "#666", fontSize: 13 }}>
                                    <li>Số tờ khai</li>
                                    <li>Mã loại hình (E41, E42, E43...)</li>
